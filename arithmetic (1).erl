...
divide(Pid, Dividend, Divisor) ->
	Pid ! {self(), Dividend, Divisor},
		receive
			Response ->
				Response
			end.
...
divider() ->
	receive
		...
		{Requesting_pid, _Dividend, 0} -> Requesting_pid ! {fail, division_by_zero};
		{Requesting_pid, Dividend, Divisor} -> Requesting_pid ! Dividend/Divisor
	end, 
	divider().

multiply(Pid, Base, Mult) ->
	Pid ! {self(), Base, Mult},
	receive
	Response ->
...
multiplier() ->
	receive
		{Pid, Base, _Mult} when (is_number(Base) == false) ->
			Pid ! {fail, Base, is_not_number};
		{Pid, _Base, Mult} when (is_number(Mult) == false) ->
			Pid ! {fail, Mult, is_not_number};
		{Pid, Base, Mult} ->
			Pid ! Base * Mult
		end,
	multiplier().

add(Pid, Length, Height) ->
	Pid ! {self(), Length, Height},
		receive
			Response ->
				Response
			end.
...
adder() ->
	receive
		{Pid, Length, _Height} when (is_number(Length) == false) -> 
			Pid ! {fail, Length, is_not_number};
		{Pid, _Length, Height} when (is_number(Height) == false) -> 
			Pid ! {fail, Height, is_not_number};
		{Pid, Length, Height} -> Pid ! Length + Height
		end,
	adder().


subtract(Pid, Length, Height) ->
	Pid ! {self(), Length, Height},
		receive
			Response ->
				Response
